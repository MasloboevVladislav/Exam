"use strict";
let application = {};//let обьявляет переменную с блочной областью видимости, переменная которая
// так обьявленна доступна только в рамках этого блока кода 
let price = {};//Если let используется внутри блока, то область перемменной ограничивается этим блоком
let sendData = new FormData(); // new создает новый экземпляр обьекта, new гарантирует что будут ссылать на данный обьект 
//FormData - это интерфейс JavaScript, который позволяет работать с данными формы HTML
const alertPlaceholder = document.getElementById('liveAlertPlaceholder'); // создаёт константу (новую именованную ссылку на область памяти), доступную только для чтения
...
const api_key = "fc15e37a-87ea-4951-afcd-969283080d3f";
// Переменные для маршрутов и пагинации
let walkingRoutes;
let currentPage = 1;
let maxPage = 1;
// Функция для обработки маршрутов 
// ("блок кода, предназначенный для выполнения определенной задачи.
// По сути функции JavaScript это то же самое, что и процедуры или подпрограммы в других языках программирования.")
function walkingRoutesHandler() {
    let url = new URL("http://exam-2023-1-api.std-900.ist.mospolytech.ru/api/routes"); // разбор url-адресса
    url.searchParams.append('api_key', api_key); // посредник между приложением и сервером.
    let xhr = new XMLHttpRequest(); //API, который предоставляет клиенту функциональность для обмена данными между клиентом и сервером, API предоставляет простой способ получения данных по ссылке без перезагрузки страницы.
